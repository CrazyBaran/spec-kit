...
> or the extension publishing guide). When it lands, add a cross-link from
> the Bitbucket section of `docs/reference/authentication.md`.
>
> **Depends on** the `bitbucket` authentication provider, which ships
> separately on `feat/add-bitbucket-auth-provider`. The public-repository
> steps work without it; the private-repository steps need it.
